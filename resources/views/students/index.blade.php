...
@section('content')
    <div class="py-10">
        ...
        @if($students->isEmpty())
            <div class="bg-white p-8 rounded-lg shadow-md text-center">
                <p class="text-gray-500 text-lg">No students found in the database.</p>
            </div>
        @else
                    ...
                    <tbody>
                        @foreach ($students as $student)
                            <tr class="border-b hover:bg-gray-50 {{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                                <td class="px-6 py-4 text-gray--cost-800">{{ $student->name }}</td>
                                <td class="px-6 py-4 text-gray-600">{{ $student->email }}</td>
                                <td class="px-6 py-4 flex space-x-3">
                                    <button onclick="openEditModal('{{ $student->id }}', '{{ $student->name }}', '{{ $student->email }}')"
                                            class="bg-blue-500 text-white px-4 py-2 rounded-lg btn hover:bg-blue-600 flex items-center">
                                        <i class="fas fa-edit mr-2"></i> Edit
                                    </button>
                                    <form action="{{ route('students.destroy', $student->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete {{ $student->name }}?');">
                                        @csrf
                                        @method('DELETE')
    ...
    <div id="addModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center hidden modal">
        <div class="bg-white p-8 rounded-xl w-full max-w-lg">
            <h2 class="text-2xl font-bold mb-6 text-gray-900">Add New Student</h2>
            <form action="{{ route('students.store') }}" method="POST">
                @csrf
                <div class="mb-5">
                    <label class="block text-gray-700 font-medium mb-2">Name</label>
                    <input type="text" name="name" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter student name">
                    @error('name')
                ...
                <div class="mb-5">
                    ...
                    <input type="email" name="email" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter student email">
                    @error('email')
                        <p class="error mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center hidden modal">
        <div class="bg-white p-8 rounded-xl w-full max-w-lg">
            <h2 class="text-2xl font-bold mb-6 text-gray-900">Edit Student</h2>
            <form id="editForm" action="" method="POST">
                ...
                <div class="mb-5">
                    <label class="block text-gray-700 font-medium mb-2">Name</label>
                    <input type="text" name="name" id="editName" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter student name">
                    @error('name')
                        <p class="error mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-5">
                    <label class="block text-gray-700 font-medium mb-2">Email</label>
                    <input type="email" name="email" id="editEmail" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter student email">
                    @error('email')
                        <p class="error mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
@section('scripts')
    <script>
        ...
        function openEditModal(id, name, email) {
            ...
            document.getElementById('editForm').action = `{{ url('students') }}/${id}`;
